...
/*another way for display but it is not working on proteus because of error in proteus
 * void display(void)
{
	unsigned long long num=hour*10000+min*100+sec;
		for(i=0;i<6;i++)
			{
			PORTA&=0xC0;
			PORTA|=(1<<(5-i));
			PORTC=(PORTC&0xF0)|(((char)(num%10))&0x0F);
			num/=10;
			_delay_ms(2);
			}
}
